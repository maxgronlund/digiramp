class LabelsController < ApplicationController
  ...
  # GET /labels.json
  def index
    PageView.create(url: '/labels' ) if request.format.to_s == 'text/html'
    @labels = Label.all
    #@users = Label.order('title').search(params[:query]).page(params[:page]).per(8)
    @labels = Label.where(activated: true).order('title').page(params[:page]).per(32)
    @user   =  current_user
  end

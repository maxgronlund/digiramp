class Account::CommonWorkIpisController < ApplicationController
  ...
  def index
    forbidden current_account_user.read_common_work_ipi?
  ...
  def new
    forbidden current_account_user.update_common_work_ipi?
    @common_work_ip = Ipi.new
  end
  
  def create
    forbidden current_account_user.create_common_work_ipi?

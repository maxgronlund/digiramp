    ...
    if @comment = Comment.create!(comment_params)
      ...
      when 'Recording'
        ...
        current_user.update_columns(
          has_wrote_a_recording_comment:    Comment.where(user_id: current_user.id, commentable_type: 'Recording').count > 0
        )
      when 'User'
        ...
        CommentMailer.delay.notify_user( @comment.id )
        current_user.update_columns(
          has_wrote_a_recording_comment:    Comment.where(user_id: current_user.id, commentable_type: 'User').count > 0
        )
      ...
      when 'Playlist'
        ...
        CommentMailer.delay.notify_user( @comment.id )
        current_user.update_columns(
          has_wrote_a_recording_comment:    Comment.where(user_id: current_user.id, commentable_type: 'Playlist').count > 0
        )
      else

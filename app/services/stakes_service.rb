...
class StakesService
  ...
  # common_work = CommonWork.last
  # ipi         = common_work.ipis.last
  # recording       = common_work.recordings.last
  # recording_ipi   = recording.recording_ipis.last
  # StakesService.update_stake(recording, recording_ipi)
  # StakesService.update_stake(common_work, ipi)

  # recording_ipi.share = 50
  # recording_ipi.save
  # StakesService.update_stake recording, recording_ipi
  ...
  def self.update_stake asset, ipi
    if ipi.share == 0.0
      delete_stake asset, ipi
    else
      set_stake asset, ipi
    end
  end
  ...
  # StakesService.delete_stake common_work, ipi
  def self.delete_stake asset, ipi
    if stake = Stake.find_by( asset_id:   asset.id,    asset_type:   asset.class.name,
                              ipiable_id: ipi.id,      ipiable_type: ipi.class.name
                            ) 
      stake.destroy
    end
  end
  ...
  # StakesService.get_stakes_for recording
  def self.get_stakes_for asset
    
    case asset.class.name
      
    when 'Recording'
      return asset.common_work.stakes + asset.stakes
    when 'CommonWork'
      return asset.common_work.stakes
    end
    
  end
  
  
  def self.representatives_split
    representative_split * Rails.application.secrets.digiramp_stripe_split.to_i * 0.01
  end
  
  private
  
  def self.set_stake asset, ipi
    
    stake = Stake.where( asset_id:   asset.id,    asset_type:   asset.class.name,
                       ipiable_id: ipi.id,      ipiable_type: ipi.class.name
                     )
                  .first_or_create( asset_id:   asset.id,    asset_type:   asset.class.name,
                                    ipiable_id: ipi.id,      ipiable_type: ipi.class.name
                                  )
    ...
    if user   = ipi.user
      stake.unassigned             = false
    else user = User.system_user
      stake.unassigned             = true
    end
    stake.email        = user.email
    stake.split        = ipi.share * 0.01
    stake.account_id   = user.account.id
    stake.save   
  end
...
#  def self.assign_stakes options = {}
#    
